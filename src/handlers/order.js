...
function register(bot) {
  ...
  bot.action(/^order:start:(.+)$/, async (ctx) => {
    ...
    await ctx.editMessageText(promptText, Markup.removeKeyboard());
  });

  // Шаг 2: Получение деталей (текст или файл)
  bot.on(['text', 'photo', 'document'], async (ctx) => {
    const order = ctx.session.order;
    if (!order || order.step !== 'waiting_details') return; // Игнорируем, если не в режиме заказа

    // Сохраняем ID сообщения, чтобы потом переслать его менеджеру (сохранит и текст, и файл!)
    order.detailMessageId = ctx.message.message_id;
    order.step = 'confirm';

    const work = catalog.getWork(workId = order.workId);
  ...
  bot.action('order:confirm', async (ctx) => {
    ...
    await ctx.reply('✅ Спасибо! Ваш заказ принят и передан менеджерам. Мы свяжемся с вами для уточнения деталей оплаты в ближайшее время.', Markup.removeKeyboard());
  });
}
