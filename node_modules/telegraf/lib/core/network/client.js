...
function register(bot) {
  ...
  bot.on(['text', 'photo', 'document'], async (ctx) => {
    // Игнорируем сообщения из групп и сообщения от других ботов
    ...
    if (order && order.step === 'waiting_details') {
      // Сохраняем ID сообщения от пользователя для пересылки менеджеру
      order.detailMessageId = ctx.message.message_id;
      
      // Сохраняем текст или тип файла для отображения в уведомлении
      if (ctx.message.text) {
  ...
  bot.action('order:confirm', async (ctx) => {
    ...
    // 🌟 Добавляем данные заказа, если они есть
    if (order.details) {
    ...
    try {
      // Отправляем ТОЛЬКО текстовое уведомление менеджеру
      await ctx.telegram.sendMessage(targetChatId, managerMsg, { parse_mode: 'Markdown' });

      // 🌟 Пересылаем ТОЛЬКО сообщение от пользователя с данными (не сообщение подтверждения!)
